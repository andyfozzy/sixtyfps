    ...
    for PicoDevices<Display, xpt2046::XPT2046<IRQ, CS, SPI>>
{
    ...
    fn read_touch_event(&mut self) -> Option<sixtyfps_corelib::input::MouseEvent> {
        self.touch
            ...
            .unwrap()
            .map(|point| {
                let point = point.to_f32() / (u16::MAX as f32);
                let size = self.display.screen_size().to_f32();
                sixtyfps_corelib::input::MouseEvent::MousePressed {
                    pos: euclid::point2(point.x * size.width, point.y * size.height),
                    button: sixtyfps_corelib::items::PointerEventButton::left,
                }
            })
            // TODO: We should keep a state and only release once
            .or(Some(sixtyfps_corelib::input::MouseEvent::MouseReleased {
                pos: Default::default(),
                button: sixtyfps_corelib::items::PointerEventButton::left,
            }))
    }
        ...
        XPT2046<IRQ, CS, SPI>
    {
        ...
        pub fn read(&mut self) -> Result<Option<Point2D<i16>>, Error<PinE, SPI::Error>> {
            if self.irq.is_low().map_err(|e| Error::Pin(e))? {
                ...
                Ok(Some(Point2D::new(y, i16::MAX - x)))
            } else {
                Ok(None)
            }
        }
