...
public:
    ...
    template<typename F>
    void set_rendering_notifier(F callback) const
    {
        auto actual_cb = [](RenderingState state, void *data) {
            (*reinterpret_cast<F *>(data))(state);
        };
        cbindgen_private::sixtyfps_windowrc_set_rendering_notifier(&inner, actual_cb,
...
public:
    ...
    void set_rendering_notifier(F callback) const
    {
        inner.set_rendering_notifier(std::move(callback));
    }
